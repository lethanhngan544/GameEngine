...
{
	PointLightPipeline::PointLightPipeline(vk::RenderPass pass, vk::DescriptorSetLayout globalDescriptorSetLayout,
		vk::ImageView position, vk::ImageView normal, vk::ImageView albedo)
	{
		...
		this->mPerLightLayout = getDevice().createDescriptorSetLayout(descLayoutCI);

		//Load shaders
		auto vertexBinary = compileShaderFromFile("shaders/fullscreen_quad.glsl", shaderc_glsl_vertex_shader);
		auto fragmentBinary = compileShaderFromFile("shaders/point.glsl", shaderc_glsl_fragment_shader);
	...
	PointLightPipeline::~PointLightPipeline()
	{
		...
		getDevice().destroyDescriptorSetLayout(mPerLightLayout);
	}

	void PointLightPipeline::begin(const vk::CommandBuffer& cmd, vk::Rect2D drawExtent) const
	{
	...
	void PointLightPipeline::processPointLight(vk::CommandBuffer cmd, const Data::PointLight& pointLight) const
	{
