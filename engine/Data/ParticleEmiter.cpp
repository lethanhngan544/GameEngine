...
namespace eg::Data
{
	...
	std::shared_ptr<ParticleEmitter::AtlasTexture> ParticleEmitter::getAtlasTexture(const std::string& textureAtlas, glm::uvec2 atlasSize)
	{
		...
		else //Load new texture
		{
			...
			auto layouts = ParticleRenderer::getTextureAtlasDescLayout();
	...
	void ParticleEmitter::record()
	{
		...
		{
			//Record particles
			ParticleRenderer::recordParticle(it->particle, mAtlasTexture->set, mAtlasTexture->size);
		}
	}
	void ParticleEmitter::update(float delta)
	{
		...
		for (auto& particle : mParticles)
		{
			...
			uint32_t index = (particle.age / mLifeSpan) * mAtlasTexture->size.x * mAtlasTexture->size.y;
