...
void main() 
{
	vec3 fragPos = getPositionFromDepth(texture(subpass0Depth, fsUv).r, fsUv);
	...
	vec3 randomVec = normalize(texture(ssaoNoise, fsUv * noiseScale).xyz);
	...
	for(int i = 0; i < SSAO_KERNEL_SIZE; ++i)
	{
		...
		vec3 fragPosSampled = getPositionFromDepth(texture(subpass0Depth, sampleUV).r, sampleUV);
	...
	// Ambient part
	vec3 fragcolor  = albedo * ubo.color * ubo.intensity * occlusion;
